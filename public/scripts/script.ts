...
const url: string = `http://localhost:3000`; // Backend's IP address with port
    ...
    await fetch(url + "/api/todos", {
        ...
        .then(todos => {
            todos.forEach((t: Todo) => {
                const newTodo : Todo =  {task: t.task, finished: t.finished};
...
async function addTodo() {
    ...
    const newTodo: Todo = {task: input, finished: false};
    todosList.push(newTodo);
...
async function renderTodo(todo: Todo) {
    ...
    newTodo.addEventListener("click", async () => {
        const idx: number = todosList.findIndex(t => t.task === newTodo.innerText);
    ...
    button.addEventListener("click", async () => {
        if (confirm("Do you want to delete this todo?")) {
            const todo: Todo =  {task: newTodo.innerText, finished: newTodo.innerHTML !== newTodo.innerText}
...
function updateProgress() {
    ...
    const progressBar = document.getElementById('relative-completion') as HTMLProgressElement;
    if (progressBar) {
